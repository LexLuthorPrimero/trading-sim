       ...
       PROGRAM-ID. ATR.
      * B-COPY + B-DEBUG + B-FSTATUS + B-NAMING + B-SMELLS
       ...
       01  WS-TR-VALUES.
           05 WS-TR-ENTRY OCCURS 1000 TIMES
              INDEXED BY WS-TR-IDX.
              10 WS-TR-VAL      PIC 9(5)V99 COMP-3.
       ...
       3100-CALCULAR-TRUE-RANGE.
           ...
           PERFORM VARYING WS-I FROM 2 BY 1
               ...
               END-IF
               MOVE WS-MAX-DIFF TO WS-TR-VAL(WS-COUNT)
               MOVE WS-CLOSE-COMP3(WS-I) TO WS-PREV-CLOSE
           END-PERFORM
           EXIT.

       3200-CALCULAR-ATR-FINAL.
           MOVE 0 TO WS-ATR
           PERFORM VARYING WS-I FROM 1 BY 1
                   UNTIL WS-I > WS-PERIOD
               ADD WS-TR-VAL(WS-I) TO WS-ATR
           END-PERFORM
           COMPUTE WS-ATR ROUNDED = WS-ATR / WS-PERIOD
           COMPUTE WS-START-IDX = WS-PERIOD + 2
           PERFORM VARYING WS-I FROM WS-START-IDX BY 1
                   UNTIL WS-I > WS-COUNT
               COMPUTE WS-ATR ROUNDED = 
                   (WS-ATR * (WS-PERIOD - 1) + WS-TR-VAL(WS-I)) 
                   / WS-PERIOD
               DISPLAY WS-ATR
           END-PERFORM
           EXIT.
